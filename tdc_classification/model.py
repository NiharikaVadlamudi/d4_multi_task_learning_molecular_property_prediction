...
class D4MP(nn.Module):
    ...
    def __init__(self,
                 ...
                 num_step_set2_set=2,
                 num_layer_set2set=1,
                 intermediate_dim=32,noclf=4,noreg=0):
        ...
        self.gat=dgl.nn.pytorch.conv.GATConv(self.node_hidden_dim,self.node_hidden_dim,num_heads=self.ntasks,allow_zero_in_degree=True,bias=True).to(device)
        ...
        for i in range(0,self.ntasks):
            ...
            nn.ELU(),
            nn.Linear(int(self.intermediate_dim/2),2).to(device),
            nn.Softmax())
            )
