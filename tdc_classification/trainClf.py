...
def get_args():
    ...
    parser.add_argument('--project_name',default='clf_tdc_task',help='name of the project')
    ...
    parser.add_argument('--learningRate',default=0.001,type=float,help='initial learning rate')
...
def create_folder(args):
    try : 
        ...
        wandb.init(project="[C] D4 Molecular Property Prediction",id=args.wid,resume='allow',config=args)
    ...
    def train(self,epoch):
        ...
        for step,samples in enumerate(self.train_loader):
            ...
            try:
                ...
                y_out=y_out.reshape((self.batchSize,1,self.ntasks,2))
                clf_loss=clf_loss_fn(y_out,labels)[:,:,:,0]
                clf_loss=clf_loss.reshape((self.batchSize,self.ntasks,1))
                loss = torch.bmm(taskMasks,clf_loss)
        ...
        # End of Epoch
        print("Train Epoch : {}  Training loss : {} Train Errors : {} ".format(epoch,(np.mean(np.array(running_loss))),countErr))
    ...
    def validate(self,epoch):
                ...
                for step,samples in enumerate(self.valid_loader):
                    try:
                        ...
                        for i,task in enumerate(tasks):
                            y_out_i=y_out[:,:,i,:].reshape((self.batchSize,1,2)).float()
                            y_gnd_i=labels[:,:,i,:].reshape((self.batchSize,1,2)).int()
            ...
            for i,task in enumerate(tasks):
                ...
                for metric in metrics :
                    finalDict['task_'+str(i)][metric]=np.mean(np.asarray(task[metric]))
